...
// 하단 컴포넌트 지연 로딩
const BattleStep = lazy(() =>
  import('@/components/Landing/BattleStep').then((m) => ({ default: m.BattleStep })),
);
const MatchingStep = lazy(() =>
  import('@/components/Landing/MatchingStep').then((m) => ({ default: m.MatchingStep })),
);
const ResultsStep = lazy(() =>
  import('@/components/Landing/ResultsStep').then((m) => ({ default: m.ResultsStep })),
);
const SpectatingStep = lazy(() =>
  import('@/components/Landing/SpectatingStep').then((m) => ({ default: m.SpectatingStep })),
);
const TierStep = lazy(() =>
  import('@/components/Landing/TierStep').then((m) => ({ default: m.TierStep })),
);
  ...
  return (
    <div className="bg-surface text-ink transition-colors duration-300 min-h-screen">
      <Meta
        ...
        description="고독한 코딩은 이제 그만! 비슷한 실력의 상대와 실시간으로 경쟁하는 알고리즘 배틀 게임 TADAK입니다. 지금 바로 참여하세요."
      />
      <main>
        {/* Hero Section */}
        ...
        <section
          id="battle-start"
          className="py-32 bg-surface relative overflow-hidden text-center"
        >
