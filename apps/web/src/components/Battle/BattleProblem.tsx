...
function BattleProblem() {
  ...
  return (
    <section className="flex flex-col gap-5 rounded-2xl bg-(--bg-layer-2) border border-border-soft p-5 text-base-primary xl:h-full xl:min-h-0 xl:overflow-y-auto">
      {!problem && (
        <div className="rounded-md border border-border-soft bg-base-primary/5 p-4 text-sm text-base-secondary">
          문제 데이터를 불러오는 중입니다. 문제가 보이지 않는다면 새로고침 해주세요.
        </div>
      )}
              ...
              {constraintItems.map((line) => (
                <li key={line.text} className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xs font-semibold text-base-secondary">
                      {line.category}
                    </span>
                    <span>{line.text}</span>
                  </div>
